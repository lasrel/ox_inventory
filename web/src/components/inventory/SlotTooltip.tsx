...
> = ({ item, inventoryType, style }, ref) => {
  ...
  const itemLabel = item.metadata?.label ? item.metadata.label : item.name ? Items[item.name]?.label || item.name : '';
      ...
      <div style={{ ...style }} className="tooltip-wrapper" ref={ref}>
        <div className="tooltip-header">
          <div className="tooltip-title">
            <p className="tooltip-name">{itemLabel}</p>
            <p className="tooltip-count">{item.count || 1}x</p>
          </div>

          <div className="tooltip-weight">
            <span className="tooltip-weight-value">{item.weight / 1000}</span>
            <span className="tooltip-weight-label">kg</span>
          </div>
        </div>

        {inventoryType === 'crafting' ? (
          <div className="tooltip-crafting">
